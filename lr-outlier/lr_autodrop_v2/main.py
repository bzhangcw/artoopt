...
def main(input_file_path, output_file_path, model_type, solver_name, x_name, y_name):
    ...
    for N in range(1, n, 2):
        ...
        y_bar = sum(y0) / len(y0)
        s_tot = sum((y0 - y_bar)**2 * q_sol)
        y_pred = [sum(x[j, i] * b_sol[j] for j in range(m)) for i in range(n)]
        s_res = sum(np.array(r_sol)**2 * q_sol)
        r_sqr = (1 - s_res / s_tot)
        ...
        s_y_pred = [y_pred[i]*(y_max-y_min)+y_min for i in selected_index]
        # d_y_pred = [y_pred[i]*(y_max-y_min)+y_min for i in dropped_index]
        beta1 = b_sol[0] * (y_max-y_min)/(x_max-x_min)
        beta0 = y_min - x_min * beta1

        # plot the results
        fig = plt.figure()
        s_x = s_x.tolist()
        d_x = d_x.tolist()
        plt.plot(s_x, s_y_pred, label="selected_regression")
